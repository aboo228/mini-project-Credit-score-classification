...
pipe3 = make_pipeline(RobustScaler(), GradientBoostingClassifier())
acc_train = None
acc_test = None
acc_all = None
# klkl
...
for i,pipe in enumerate(tqdm([pipe1, pipe2])):
    ...
    train_predict[f'pipe+{i}']=np.argmax(pipe.predict(x_train), axis=1)
    test_predict[f'pipe+{i}']=np.argmax(pipe.predict(x_test), axis=1)
    all_predict[f'pipe+{i}']=np.argmax(pipe.predict(train_dfall.iloc[:, 1:-1]), axis=1)
    acc_train = (train_predict[f'pipe+{i}'] == np.argmax(y_train.to_numpy(), axis=1)).sum()/y_train.shape[0]
    acc_test = (test_predict[f'pipe+{i}'] == np.argmax(y_test.to_numpy(), axis=1)).sum() / y_test.shape[0]
    acc_all = (all_predict[f'pipe+{i}'] == np.argmax(targetall.to_numpy(),axis=1)).sum() / targetall.shape[0]
...
acc_train = (train_predict['pipe3'] == y_train.to_numpy()).sum() / y_train.shape[0]
acc_test = (test_predict['pipe3'] == y_test.to_numpy()).sum() / y_test.shape[0]
