...
print(train_df[train_df.loc[:, 'Age'] == 111])
...
for column in range(len(roll_columns)):
    for instance, i in tqdm(zip(invalid_values_instances[column], invalid_index_by_cust_id[column])):
        fill = None
        if 0 < instance % 8 < 7: #?
...
train_df.drop(['Customer_ID', 'Month', 'Payment_Behaviour','ID', 'Name', 'SSN','Type_of_Loan'], axis=1, inplace=True)
#
#
...
#todo replace the num coulumns in name columns
